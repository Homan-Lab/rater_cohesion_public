...
class xRR:
    ...
    def __init__(self, X, Y, answer_col, rater_col, object_col, dist_func=categorical):
        ...
        if len(X[object_col].unique()) != len(Y[object_col].unique()):
            ...
            valid_objects = np.intersect1d(
                X[object_col].unique(), Y[object_col].unique()
            )
        ...
        self.X_dict = {
            k: {k_: v_ for k_, v_ in v.items() if not np.isnan(v_)}
        ...
        self.n = len(self.X[object_col].unique())
        # self.n = len(self.dict_keys)
        # print(self.n)
        self.kappa = None

    def kappa_x(self, workers=1):
        ...
        """
        if not self.kappa:
            self.kappa = 1 - (self.d_o(workers=workers) / self.d_e(workers=workers))
    ...
    def do_inner(self, i):
        ...
        do_i = 0
        sum_rs = self.R[i] + self.S[i]
        prod_rs = self.R[i] * self.S[i]
        for r in self.X_dict[self.dict_keys[i]].values():
            for s in self.Y_dict[self.dict_keys[i]].values():
                do_i += self.dist_func(r, s)
